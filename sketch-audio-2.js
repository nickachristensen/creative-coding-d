...
const sketch = () => {
  ...
  for (let i = 0; i < numCircles; i++) {
    rotationsOffsets.push(random.range(Math.PI * -0.25, Math.PI * 0.25) - Math.PI * 0.5);
  }
  ...
  return ({ context, width, height }) => {
    ...
    for (let i = 0; i < numCircles; i++) {
      context.save();
      context.rotate(rotationsOffsets[i]);
...
const createAudio = () => {
  ...
  analyserNode.smoothingTimeConstant = 0.955;
